...
pygame.init()
# Bazowe ustawienia (używane do skalowania)
BASE_WIDTH, BASE_HEIGHT = 900, 660
BASE_GRID_SIZE = 10
BASE_TILE_SIZE = 50
BASE_BOARD_OFFSET_Y = 60
BASE_BOARD_OFFSET_X = 60
BASE_FONT_SIZE = 36
BASE_BUTTON_FONT = 24
BASE_HEADER_Y = 30

# Początkowe wymiary okna (mogą się zmieniać)
...
# Set up the screen (resizable + double buffer)
screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.DOUBLEBUF | pygame.RESIZABLE)
pygame.event.set_allowed([pygame.QUIT, pygame.MOUSEBUTTONDOWN, pygame.KEYDOWN, pygame.MOUSEMOTION, pygame.VIDEORESIZE])
...
for word in aword_list:
    if len(word) > 1 and all(char in kanji_list for char in word):
        kword_list.append(word)

# Zmienne skalowane - zainicjalizowane dalej przez update_scale()
SCALE = 1.0
TILE_SIZE = BASE_TILE_SIZE
BOARD_OFFSET_X = BASE_BOARD_OFFSET_X
BOARD_OFFSET_Y = BASE_BOARD_OFFSET_Y
FONT = get_font(BASE_FONT_SIZE)
FONT_SMALL = get_font(16)
BUTTON_FONT = get_font(BASE_BUTTON_FONT)
HEADER_Y = BASE_HEADER_Y

def update_scale(new_w, new_h):
    global WIDTH, HEIGHT, SCALE, TILE_SIZE, BOARD_OFFSET_X, BOARD_OFFSET_Y, FONT, FONT_SMALL, BUTTON_FONT, HEADER_Y
    WIDTH, HEIGHT = new_w, new_h
    # jednolita skala by wszystko zachowywało proporcje
    SCALE = min(max(0.5, WIDTH / BASE_WIDTH), max(0.5, HEIGHT / BASE_HEIGHT))
    TILE_SIZE = max(16, int(BASE_TILE_SIZE * SCALE))
    BOARD_OFFSET_X = int(BASE_BOARD_OFFSET_X * SCALE)
    BOARD_OFFSET_Y = int(BASE_BOARD_OFFSET_Y * SCALE)
    FONT = get_font(max(12, int(BASE_FONT_SIZE * SCALE)))
    FONT_SMALL = get_font(max(10, int(16 * SCALE)))
    BUTTON_FONT = get_font(max(12, int(BASE_BUTTON_FONT * SCALE)))
    HEADER_Y = max(10, int(BASE_HEADER_Y * SCALE))

# Pierwsze ustawienie skalowania
...
def draw_score():
    ...
    score_rect = score_surface.get_rect(center=(WIDTH // 2, HEADER_Y))
    screen.blit(score_surface, score_rect)

def draw_button(x, y, width, height, text, color, text_color):
    pygame.draw.rect(screen, color, (x, y, width, height))
    pygame.draw.rect(screen, BLACK, (x, y, width, height), max(1, int(2 * SCALE)))
...
def end_game():
    ...
    while True:
        ...
        final_score_rect = final_score_text.get_rect(center=(WIDTH // 2, int(100 * SCALE)))
        screen.blit(final_score_text, final_score_rect)

        new_game_button = draw_button(WIDTH // 2 - int(75 * SCALE), HEIGHT // 2 + int(120 * SCALE), int(150 * SCALE), int(50 * SCALE), "New Game", GREEN, WHITE)
        close_button = draw_button(WIDTH // 2 - int(75 * SCALE), HEIGHT // 2 + int(190 * SCALE), int(150 * SCALE), int(50 * SCALE), "Quit", RED, WHITE)
    ...
    for word in new_words:
        if word in kword_list:
            ...
            if word_positions:
                ...
                # Use semi-transparent yellow
                semi_yellow = (YELLOW[0], YELLOW[1], YELLOW[2], 120)
                ...
                for (row, col) in word_positions:
                    ...
                    pygame.draw.rect(flash_surface, semi_yellow, rect)

                # Display the flash effect
                screen.blit(original_surface, (0, 0))  # Restore the original screen
                screen.blit(flash_surface, (0, 0))  # Overlay the semi-transparent flash
...
def get_word_info_cached(word):
    ...
    try:
        response = session.get(f"https://jisho.org/api/v1/search/words?keyword={word}", timeout=5)
...
def draw_word_details(word, readings, meaning):
    pygame.draw.rect(screen, BLACK, (0, HEIGHT - int(100 * SCALE), WIDTH, int(100 * SCALE)))

    kanji_surface = FONT.render(word, True, WHITE)
    kanji_rect = kanji_surface.get_rect(topleft=(int(100 * SCALE), HEIGHT - int(80 * SCALE)))
    screen.blit(kanji_surface, kanji_rect)

    furigana_surface = get_font(max(12, int(16 * SCALE))).render(", ".join(readings), True, WHITE)
    furigana_rect = furigana_surface.get_rect(midbottom=(kanji_rect.centerx, kanji_rect.top + int(10 * SCALE)))
    screen.blit(furigana_surface, furigana_rect)

    meaning_surface = get_font(max(14, int(20 * SCALE))).render(f"- {meaning}", True, WHITE)
    meaning_x = kanji_rect.midright[0] + int(10 * SCALE)
...
def draw_right_words():
    ...
    displayed_word_rects = []

    right_x = WIDTH - int(200 * SCALE)
    for word in displayed_words:
        word_surface = FONT.render(word, True, WHITE)
        word_rect = screen.blit(word_surface, (right_x, y_offset))
        displayed_word_rects.append((word, word_rect))

        if word_rect.collidepoint(mouse_x, mouse_y):
            pygame.draw.rect(screen, BRIGHT_YELLOW, word_rect.inflate(int(10 * SCALE), int(10 * SCALE)), max(1, int(2 * SCALE)))
...
def draw_centered_words():
    ...
    for i, word in enumerate(displayed_words):
        ...
        x_offset = start_x + col * column_offset
        y_offset = start_y + row * int(60 * SCALE)
        ...
        if word_rect.collidepoint(mouse_x, mouse_y):
            ...
            furigana_surface = get_font(max(12, int(16 * SCALE))).render(", ".join(hovered_word_data_cache[word][0]), True, WHITE)
            furigana_rect = furigana_surface.get_rect(midbottom=(kanji_rect.centerx, kanji_rect.top + int(10 * SCALE)))
            screen.blit(furigana_surface, furigana_rect)
...
def game_loop():
    global score, valid_words, selected_tiles, active_word, WIDTH, HEIGHT, screen
    board, valid_words = generate_board()
    # Zaktualizuj skalę na wypadek, gdy rozmiary były zmienione
    update_scale(WIDTH, HEIGHT)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Ustaw nowy rozmiar i przeskaluj elementy
                WIDTH_NEW, HEIGHT_NEW = event.w, event.h
                update_scale(WIDTH_NEW, HEIGHT_NEW)
                screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.DOUBLEBUF | pygame.RESIZABLE)
...
def generate_board():
    ...
    # Use min to avoid ValueError if not enough words
    count = min(20, max(0, len(kword_list)))
    words_to_place = random.sample(kword_list, count) if count > 0 else []
    placed_words = []
    ...
    # Fill remaining empty spaces with random kanji
    for row in range(GRID_SIZE):
        for col in range(GRID_SIZE):
